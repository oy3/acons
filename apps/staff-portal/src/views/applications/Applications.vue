...
export default {
  ...
  methods: {
    ...
    getApplicationProgramDisplay(application) {
      const programType =
        application?.programTypeLabel || this.getProgramTypeLabel(application);
      const studyMode =
        application?.programModeLabel || this.getStudyModeLabel(application);
      const programName = this.getProgramName(application);

      return (
        [programType, studyMode, programName]
    ...
    getProgramTypeLabel(application) {
      const programType =
        application?.programId?.programTypeId || application?.programTypeId;
      if (!programType) return "N/A";
      if (typeof programType === "string")
        return application?.programTypeLabel || "N/A";
      return (
        programType.type ||
        programType.name ||
    ...
    getStudyModeLabel(application) {
      const studyMode =
        application?.programId?.programModeId || application?.programModeId;
      if (!studyMode) return "N/A";
      if (typeof studyMode === "string")
        return application?.programModeLabel || "N/A";
      return (
    ...
    getProfileImage(application) {
      return (
        ...
        "https://placehold.co/160x160?text=Applicant"
      );
    },
    ...
    getDocumentSections(application) {
      ...
      const olevelResults = Array.isArray(application.documents.olevelResults)
        ...
        : [];
      const referenceLetters = Array.isArray(
      ...
      if (olevelResults.length) {
        sections.push({
          ...
          documents: olevelResults.map((document, index) => ({
            label: document.type
              ? this.formatLabel(document.type)
              : `Result ${index + 1}`,
            url: document.url,
    ...
    getDocumentCount(application) {
      return this.getDocumentSections(application).reduce(
    ...
    canReviewManualTransfer(payment) {
      return (
        this.isManualTransferPayment(payment) && payment?.status === "pending"
      );
    },
    ...
    async reloadSelectedApplicationDetails() {
      ...
      if (!response.success || !response.data?.application) {
        throw new Error(
          response.message || "Failed to refresh application details",
        );
      }

      this.selectedApplication = response.data.application;
      this.selectedPaymentHistory =
        response.data.paymentHistory || createEmptyPaymentHistory();
    },
    ...
    async sendMatriculationEmail(application) {
      try {
        ...
        // Make API call
        const response = await apiService.sendMatriculationEmail(
          application.id,
        );
        ...
        this.$swal.fire({
          ...
          text:
            error.message ||
            "An error occurred while sending the matriculation email",
          confirmButtonColor: "#dc3545",
        });
      }
    },
    ...
    async generateMatriculationNumber(application) {
      try {
        ...
        // Make API call
        const response = await apiService.generateMatriculationNumber(
          application.id,
        );
        ...
        this.$swal.fire({
          ...
          text:
            error.message ||
            "An error occurred while recovering the matriculation number",
          confirmButtonColor: "#dc3545",
        });
      }
    },
...
<template>
  <div class="container-fluid p-4">
                        ...
                        <div class="flex-grow-1">
                          ...
                          >
                            ...
                            <span
                              class="badge rounded-pill"
                              :class="
                                getStatusBadgeClass(selectedApplication.status)
                              "
                            >
                              {{
                          ...
                          <div class="d-flex flex-wrap gap-2 mb-3">
                            ...
                            <span class="detail-chip">
                              ...
                              >{{
                                getAcademicSessionLabel(selectedApplication)
                              }}
                            </span>
                          </div>
                          ...
                          <div class="details-grid compact-grid">
                            <div>
                              ...
                              <span class="details-value">{{
                                getStageLabel(selectedApplication.currentStage)
                              }}</span>
                            </div>
                            <div>
                              <span class="details-label">Age</span>
                              <span class="details-value">{{
                                calculateAge(selectedApplication.dob)
                              }}</span>
                            </div>
                            <div>
                      ...
                      <div class="summary-panel h-100">
                        <div class="summary-stat">
                          <span class="summary-value">{{
                            getDocumentCount(selectedApplication)
                          }}</span>
                          <span class="summary-label">Documents</span>
                        </div>
                        <div class="summary-stat">
                          <span class="summary-value">{{
                            selectedPaymentHistory.totalCount
                          }}</span>
                          <span class="summary-label">Payments</span>
                        </div>
                        <div class="summary-stat">
                          <span class="summary-value">{{
                            formatCurrency(selectedPaymentHistory.totalPaid)
                          }}</span>
                          <span class="summary-label">Successful Payments</span>
                        </div>
                        <div class="summary-stat">
                          <span class="summary-value">{{
                            selectedApplication.matriculationNumber || "Pending"
                          }}</span>
                ...
                <div class="col-lg-8">
                    ...
                    <div class="card-body">
                      ...
                      <div class="details-grid">
                        ...
                        <div>
                          ...
                          <span class="details-value">{{
                            formatLabel(selectedApplication.userId?.role)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Program Type</span>
                          <span class="details-value">{{
                            getProgramTypeLabel(selectedApplication)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Study Mode</span>
                          <span class="details-value">{{
                            getStudyModeLabel(selectedApplication)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">State of Origin</span>
                          <span class="details-value">{{
                            selectedApplication.stateOfOrigin || "N/A"
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Local Government</span>
                          <span class="details-value">{{
                            selectedApplication.lga || "N/A"
                          }}</span>
                        </div>
                        <div>
                      ...
                      <div class="row g-4">
                        <div class="col-md-6">
                          <div class="info-block h-100">
                            ...
                            <p class="mb-1">
                              <strong>Name:</strong>
                              {{ selectedApplication.guardian?.name || "N/A" }}
                            </p>
                            <p class="mb-1">
                              <strong>Phone:</strong>
                              {{ selectedApplication.guardian?.phone || "N/A" }}
                            </p>
                            <p class="mb-1">
                              <strong>Email:</strong>
                              {{ selectedApplication.guardian?.email || "N/A" }}
                            </p>
                            <p class="mb-1">
                              <strong>Relationship:</strong>
                              {{
                                selectedApplication.guardian?.relationship ||
                                "N/A"
                              }}
                            </p>
                            <p class="mb-0">
                              ...
                              {{
                                selectedApplication.guardian?.address || "N/A"
                              }}
                            </p>
                          </div>
                        </div>
                        <div class="col-md-6">
                          <div class="info-block h-100">
                            ...
                            <p class="mb-1">
                              <strong>Name:</strong>
                              {{ selectedApplication.nextOfKin?.name || "N/A" }}
                            </p>
                            <p class="mb-1">
                              <strong>Phone:</strong>
                              {{
                                selectedApplication.nextOfKin?.phone || "N/A"
                              }}
                            </p>
                            <p class="mb-1">
                              <strong>Email:</strong>
                              {{
                                selectedApplication.nextOfKin?.email || "N/A"
                              }}
                            </p>
                            <p class="mb-1">
                              <strong>Relationship:</strong>
                              {{
                                selectedApplication.nextOfKin?.relationship ||
                                "N/A"
                              }}
                            </p>
                            <p class="mb-0">
                              <strong>Address:</strong>
                              {{
                                selectedApplication.nextOfKin?.address || "N/A"
                              }}
                            </p>
                          </div>
                        </div>
                  ...
                  <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                      ...
                      <div class="row g-4">
                        <div class="col-md-6">
                          <div class="info-block h-100">
                            ...
                            <p class="mb-1">
                              ...
                              {{
                                selectedApplication.academicBackground?.primary
                                  ?.name || "N/A"
                              }}
                            </p>
                            <p class="mb-1">
                        ...
                        <div class="col-md-6">
                          <div class="info-block h-100">
                            <h6 class="info-block-title">
                              Secondary Education
                            </h6>
                            <p class="mb-1">
                              <strong>School:</strong>
                              {{
                                selectedApplication.academicBackground
                                  ?.secondary?.name || "N/A"
                              }}
                            </p>
                            <p class="mb-1">
                    ...
                    <div class="card-body">
                      ...
                      >
                        ...
                        <span class="badge bg-light text-dark"
                          >{{
                            selectedApplication.examinations?.length || 0
                          }}
                          records</span
                        >
                      </div>
                      ...
                      <div
                        v-if="selectedApplication.examinations?.length"
                        class="d-flex flex-column gap-3"
                      >
                        <div
                          v-for="(
                            exam, examIndex
                          ) in selectedApplication.examinations"
                          :key="`${exam.examType}-${examIndex}`"
                          class="info-block"
                        >
                          <div
                            class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2"
                          >
                            <h6 class="info-block-title mb-0">
                              {{ exam.examType || "Exam Record" }}
                            </h6>
                            <span class="text-muted small"
                              >{{ exam.examYear || "N/A" }} ·
                              {{ exam.examNumber || "No Number" }}</span
                            >
                          </div>
                          <div
                            v-if="exam.subjects?.length"
                            class="d-flex flex-wrap gap-2"
                          >
                            <span
                              v-for="(subject, subjectIndex) in exam.subjects"
                              :key="`${subject.subject}-${subjectIndex}`"
                              class="detail-chip soft-chip"
                            >
                              {{ subject.subject || "Subject" }}:
                              {{ subject.grade || "N/A" }}
                            </span>
                          ...
                          <p v-else class="text-muted mb-0">
                            No subject breakdown submitted.
                          </p>
                        </div>
                      </div>
                      <p v-else class="text-muted mb-0">
                  ...
                  <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body">
                      ...
                      <div
                        v-if="selectedApplication.referees?.length"
                        class="row g-3"
                      >
                        <div
                          v-for="(
                            referee, refereeIndex
                          ) in selectedApplication.referees"
                          :key="`${referee.email}-${refereeIndex}`"
                          class="col-md-6"
                        >
                          <div class="info-block h-100">
                            <h6 class="info-block-title">
                              {{
                                referee.name || `Referee ${refereeIndex + 1}`
                              }}
                            </h6>
                            <p class="mb-1">
                              <strong>Email:</strong>
                    ...
                    <div class="card-body">
                      ...
                      <div class="details-grid single-column-grid">
                        ...
                        <div>
                          ...
                          <span class="details-value">
                            {{
                              selectedApplication.admissionFlow
                    ...
                    <div class="card-body">
                      ...
                      <div class="details-grid single-column-grid">
                        <div>
                          ...
                          <span class="details-value">{{
                            formatDate(selectedApplication.entranceExam?.date)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Time</span>
                          <span class="details-value">{{
                            selectedApplication.entranceExam?.time || "N/A"
                          }}</span>
                        </div>
                        <div>
                    ...
                    <div class="card-body">
                      ...
                      <div class="details-grid single-column-grid">
                        <div>
                          <span class="details-label">Date</span>
                          <span class="details-value">{{
                            formatDate(selectedApplication.screening?.date)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Time</span>
                          <span class="details-value">{{
                            selectedApplication.screening?.time || "N/A"
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Venue</span>
                          <span class="details-value">{{
                            selectedApplication.screening?.venue || "N/A"
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Completed</span>
                          <span class="details-value">{{
                            selectedApplication.screening?.completed
                              ? "Yes"
                              : "No"
                          }}</span>
                        </div>
                    ...
                    <div class="card-body">
                      ...
                      <div class="details-grid single-column-grid">
                        <div>
                          ...
                          <span class="details-value">{{
                            getAdmissionDecisionValue(selectedApplication)
                          }}</span>
                        </div>
                        <div>
                          <span class="details-label">Admission Date</span>
                          <span class="details-value">{{
                            formatDate(selectedApplication.admissionDate)
                          }}</span>
                        </div>
                        <div>
                          ...
                          <span v-else class="details-value"
                            >Not generated</span
                          >
                        </div>
                    ...
                    <div class="card-body">
                      ...
                      <div class="payment-summary-grid">
                        <div class="summary-tile success-tile">
                          <span class="summary-value">{{
                            selectedPaymentHistory.successfulCount
                          }}</span>
                          <span class="summary-label">Successful</span>
                        </div>
                        <div class="summary-tile warning-tile">
                          <span class="summary-value">{{
                            selectedPaymentHistory.pendingCount
                          }}</span>
                        ...
                        <div class="summary-tile secondary-tile">
                          <span class="summary-value">{{
                            selectedPaymentHistory.cancelledCount
                          }}</span>
              ...
              <div class="card border-0 shadow-sm">
                <div class="card-body">
                  ...
                  >
                    ...
                    <span class="badge bg-light text-dark"
                      >{{ selectedPaymentHistory.totalCount }} records</span
                    >
                  </div>

                  <div
                    v-if="selectedPaymentHistory.payments.length"
                  ...
                  >
                    <table class="table table-hover align-middle mb-0">
                      ...
                      <tbody>
                        <tr
                          v-for="payment in selectedPaymentHistory.payments"
                          :key="payment.id"
                        >
                          <td>
                            <div class="fw-semibold">
                              {{ payment.payment?.name || "Unknown Payment" }}
                            </div>
                            <div class="small text-muted">
                              {{ payment.payment?.paymentCode || "No code" }}
                            </div>
                          </td>
                          <td>
                          ...
                          <td>
                            ...
                            <div class="small text-muted" v-if="payment.fee">
                          ...
                          <td>
                            ...
                            <div
                              class="small text-muted"
                              v-if="payment.verificationRemarks"
                            >
                              {{ payment.verificationRemarks }}
                            </div>
                          </td>
                          <td>
                            <div
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  <span
                                    v-if="isProcessingPayment(payment.id)"
                  ...
                  <p v-else class="text-muted mb-0">
                    No payment records linked to this applicant yet.
                  </p>
                </div>
              </div>
            </div>
    ...
    >
      <div
        class="modal-dialog modal-xl modal-dialog-centered receipt-preview-dialog"
      >
          ...
          <div class="modal-header border-0 pb-0">
            <div>
              ...
              <p v-if="selectedPaymentReceipt" class="text-muted mb-0 small">
                {{ selectedPaymentReceipt.payment?.name || "Payment Receipt" }}
                · {{ getReceiptFilename() }}
              </p>
            </div>
            <button
              type="button"
              class="btn-close"
              @click="closePaymentReceiptModal"
            ></button>
          </div>
          ...
          <div class="modal-body pt-3" v-if="selectedPaymentReceipt">
            <div class="receipt-preview-shell">
              ...
              <div v-else class="receipt-preview-fallback text-center">
                <i
                  class="bi bi-file-earmark-text fs-1 mb-3 d-block text-muted"
                ></i>
    ...
    <div
      ...
      :class="{ show: showDetailsModal }"
      v-if="showDetailsModal && !showPaymentReceiptModal"
    ></div>
    <div
      class="modal-backdrop fade receipt-preview-backdrop"
      :class="{ show: showPaymentReceiptModal }"
      v-if="showPaymentReceiptModal"
      @click="closePaymentReceiptModal"
    ></div>
  </div>
</template>
