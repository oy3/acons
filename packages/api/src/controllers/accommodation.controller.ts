import { Body, Controller, ForbiddenException, Get, Param, Post, Query, Request, Res, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
import { FileInterceptor } from '@nestjs/platform-express';
import { ApiBearerAuth, ApiTags } from '@nestjs/swagger';
import { AccommodationService } from '../services/accommodation.service';
import { CompleteExternalAccommodationDto, CreateHostelBlockDto, CreateHostelDto, CreateHostelRoomDto, ManualAllocationDto, SignExternalTenancyAgreementDto, StartExternalAccommodationDto, UpdateAccommodationInventoryDto } from '../dto/accommodation.dto';
...
export class PublicAccommodationController {
    ...
    submitManualTransfer(
        ...
        return this.accommodationService.submitExternalManualTransfer(token, receipt);
    }

    @Post('payment/verify')
    verifyPayment(@Body() body: { resumeToken: string; reference: string }) {
        return this.accommodationService.verifyExternalPayment(body.resumeToken, body.reference);
    }
    ...
    async downloadDocument(
        ...
        const document = await this.accommodationService.downloadExternalDocument(resumeToken, documentType);
...
export class StaffAccommodationController {
    ...
    private async authorize(req: any, permission: string) {
        ...
        const access = await this.rolesService.getUserModuleAccess(this.actorId(req), 'accommodation');
        if (!access || (!access.permissions.includes(permission) && !access.permissions.includes('manage'))) {
            throw new ForbiddenException('You do not have the required accommodation permission');
        }
    }
    ...
    async updateHostelStatus(@Request() req, @Param('id') id: string, @Body() body: UpdateAccommodationInventoryDto) {
        await this.authorize(req, 'configure');
        return this.accommodationService.updateInventoryStatus('hostel', id, body.active, this.actorId(req));
    }

    @Post('blocks/:id/status')
    async updateBlockStatus(@Request() req, @Param('id') id: string, @Body() body: UpdateAccommodationInventoryDto) {
        await this.authorize(req, 'configure');
        return this.accommodationService.updateInventoryStatus('block', id, body.active, this.actorId(req));
    }

    @Post('rooms/:id/status')
    async updateRoomStatus(@Request() req, @Param('id') id: string, @Body() body: UpdateAccommodationInventoryDto) {
        await this.authorize(req, 'configure');
        return this.accommodationService.updateInventoryStatus('room', id, body.active, this.actorId(req));
    }

    @Post('applications/:id/allocate')
    async allocate(@Request() req, @Param('id') id: string, @Body() body: ManualAllocationDto) {
        await this.authorize(req, 'allocate');
        return this.accommodationService.manualAllocate(id, body.roomId, body.slotNumber, this.actorId(req), body.note);
    }
